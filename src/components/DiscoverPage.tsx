...
function mapDiscoveryListing(
  ...
  const category =
    categoryItems.find(
      (catalogItem) =>
        catalogItem.id === item.category_id
    )?.name || 'Clothing'

  const brand =
    brandItems.find(
      (catalogItem) =>
        catalogItem.id === item.brand_id
    )?.name || 'Local Brand'
  ...
  return {
    ...
    // Backend field is optional, Product requires string.
    size: item.size ?? 'N/A',
    ...
    // Backend field is optional, Product requires number.
    ...
    seller: {
      // seller can be missing from the API response.
      id: item.seller?.id ?? '',
    ...
    description:
      item.description ||
      'A community-listed pre-loved garment.',
...
> = ({
  ...
  /*
   * Load categories and brands from backend.
   */
  useEffect(() => {
    let isMounted = true
    ...
    Promise.all([
      ...
      .catch(() => {
        /*
         * Keep local filter options available
         * if the catalog API is unavailable.
         */
      })
  ...
  /*
   * Load listings whenever filters/catalog data change.
   */
  useEffect(() => {
    let isMounted = true
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            'linear-gradient(135deg, #203D43 0%, #2A4F56 100%)',
          color: '#fff',
          padding: '24px',
          marginBottom: '24px',
          display: 'flex',
      ...
      >
        ...
        >
          ...
          <option value="lowest_points">
            Sort: Lowest Points
          </option>

          <option value="best_rated">
            Sort: Best Rated Seller
          </option>
